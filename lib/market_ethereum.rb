      ...
      if bid.side != ask.side &&
        ...
        users.each do |user|
          ...
          if user.user_id == ask.user_id
            if user.eth_currency_balance - order.amount >= 0
              user.eth_currency_balance = user.eth_currency_balance - order.amount
              user.quote_currency_balance = user.quote_currency_balance + bid.amount * bid.price - bid_fee / 2
              user.close_order(ask.order_id)
            else
              puts "Insufficient eth_currency_balance \n"
              puts user.eth_currency_balance.round(8).to_s("F")
        ...
        orders.each do |order|
          puts "Order number #{order.order_id} has #{order.amount.to_s("F")} ETH and #{order.price.to_s("F")} EUR"
        end
